...
class BaseModel(models.Model):
    ...
    name = models.CharField(
            ...
            help_text = "name or title", blank=False)
...
class Consultation(BaseModel):
    ...
    status = models.CharField(
            max_length=15, choices=CONS_CHOICES)

    def __str__(self):
        return "{} by {} with satatus{}".format(
                self.name, self.user.username, self.status)
...
class FeedBack(BaseModel):
    ...
    def __str__(self):
        return "{} Rating by {}".format(
                self.rating, self.consultation)
...
class Officer(BaseModel):
    ...
    portfolio = models.CharField(max_length=150, help_text="Brief Educational Portfolio", blank=False)
    def __str__(self):
        return self.name
